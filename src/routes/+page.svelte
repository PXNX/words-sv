...
<main class:celebrating={celebration} class="game-shell">
  ...
  <section class="game-paper" aria-label={labels.label}>
    ...
    <div class="selection-area" aria-live="polite"><span class="round-chip">{labels.round} {roundNumber}</span><button class:open={settingsOpen} class="settings-trigger" aria-expanded={settingsOpen} aria-controls="game-settings" onclick={() => (settingsOpen = !settingsOpen)}>{#if settingsOpen}<IconClose />{:else}<IconSettings />{/if}<span class="sr-only">{settingsOpen ? 'Close' : labels.settings}</span></button><div class:has-word={previewWord.length > 0} class:correct={feedback === 'correct'} class:wrong={feedback === 'wrong'} class="selected-word"><span>{previewWord}</span>{#if feedback === 'correct'}<IconCheck aria-label="Correct" />{:else if feedback === 'wrong'}<IconClose aria-label="Incorrect" />{/if}</div></div>
