  ...
  <section class="game-paper" aria-label={labels.label}>
    {#if settingsOpen}
      <aside id="game-settings" class="settings-panel" aria-label={labels.settings} style={`--settings-texture: url(${settingsTexture});`}>
        ...
        <a class="settings-link" href="/word-data-attribution.txt" target="_blank" rel="noreferrer">{labels.sourceNote}</a><a class="settings-link settings-github" href="https://github.com/PXNX/words-sv" target="_blank" rel="noreferrer"><IconGithub /><span>GitHub · PXNX/words-sv</span></a>
      </aside>
    {/if}
