...
const hintableBaseWords = new Set([...Object.keys(wordDefinitions.de), ...Object.keys(wordDefinitions.en)]);
...
let revealedHintWord = $state<string | null>(null);
  ...
  function newRound(nextLanguage = lang, resetCount = false) {
	...
	function selectLanguage(nextLanguage: Language) { newRound(nextLanguage, true); if (interfaceLocale !== 'fa') interfaceLocale = nextLanguage; settingsOpen = false; }
		...
		function goHome() { settingsOpen = false; tutorialOpen = false; void goto('/'); }
...
<svelte:window onpointerup={endSwipe} onpointercancel={endSwipe} />
    ...
    {#if settingsOpen}
      <aside id="game-settings" class="settings-panel" aria-label={labels.settings} dir={interfaceDirection} lang={interfaceLocale}>
	    ...
	    {#if !settingsOpen}<button class="home-trigger" onclick={goHome} aria-label={labels.homeButton}><IconHome aria-hidden="true" /><span>{labels.homeButton}</span></button>{/if}
	    {#if !settingsOpen && gameMode !== 'crossword'}<button class="settings-trigger mode-settings-trigger" aria-expanded="false" aria-controls="game-settings" onclick={() => (settingsOpen = true)}><IconSettings aria-hidden="true" /><span class="sr-only">{labels.settings}</span></button>{/if}
    ...
    <div class:install-ready={installPrompt && !previewWord && !celebration} class:completion-area={celebration} class="selection-area" aria-live="polite">
      ...
      {:else}
        ...
        {#if idleHintReady && !settingsOpen && !tutorialOpen}<button class="idle-hint-trigger" onclick={showIdleHint}><IconHelp aria-hidden="true" /><span>{labels.idleHint}</span></button>{/if}
...
<style>
  ...
  .brand-mark { position:relative;width:34px;height:34px;display:block;flex:none; }.brand-mark i,.brand-mark b { position:absolute;display:block;width:21px;height:21px;border:2px solid #172a45;border-radius:50%; }.brand-mark i { top:1px;left:1px; }.brand-mark b { right:1px;bottom:1px;border-color:#e6a527; }
	  .settings-trigger,.settings-close { display:grid;place-items:center;width:2.15rem;height:2.15rem;border:1px solid rgba(23,42,69,.24);border-radius:50%;background:rgba(255,253,247,.86);color:#172a45;transition:transform .18s cubic-bezier(.23,1,.32,1),background .18s ease; }.settings-trigger :global(svg),.settings-close :global(svg) { width:1.1rem;height:1.1rem; }.settings-close { position:absolute;z-index:2;top:.72rem;right:.72rem;background:#172a45;color:#fffdf7; }.settings-close:active { transform:scale(.94); }.mode-settings-trigger { position:absolute;z-index:100;top:.62rem;right:.62rem; }.home-trigger { position:absolute;z-index:105;top:.62rem;left:.62rem;display:inline-flex;align-items:center;gap:.32rem;min-height:2.1rem;padding:0 .62rem;border:1px solid rgba(23,42,69,.36);border-radius:999px;background:rgba(255,253,247,.94);color:#172a45;font-family:'DM Sans',sans-serif;font-size:.58rem;font-weight:900;letter-spacing:.07em;text-transform:uppercase;box-shadow:0 2px 0 rgba(23,42,69,.1); }.home-trigger :global(svg) { width:1rem;height:1rem; }.home-trigger:active { transform:scale(.96); }
  ...
  @keyframes completion-in { from { opacity: 0; transform: scale(.94); } to { opacity: 1; transform: scale(1); } }
	  :global(html.dark) .game-paper { background:rgba(23,42,69,.96);border-color:rgba(255,253,247,.18);box-shadow:0 24px 70px rgba(0,0,0,.35); }:global(html.dark) .game-paper::before { border-color:rgba(255,253,247,.14); }:global(html.dark) .tutorial-panel,:global(html.dark) .tutorial-card { background:#172a45;color:#fffdf7; }:global(html.dark) .tutorial-card { border-color:rgba(255,253,247,.32); }:global(html.dark) .tutorial-card h1,:global(html.dark) .tutorial-steps li { color:#fffdf7; }:global(html.dark) .tutorial-steps b { border-color:#fffdf7;background:#172a45;color:#e6a527; }:global(html.dark) .tutorial-start { border-color:#e6a527;background:#e6a527;color:#172a45; }:global(html.dark) .wheel-stage { border-color:rgba(255,253,247,.18); }:global(html.dark) .settings-trigger,:global(html.dark) .setting-row,:global(html.dark) .selected-word.has-word { color:#fffdf7; }:global(html.dark) .home-trigger { border-color:rgba(255,253,247,.42);background:rgba(23,42,69,.94);color:#fffdf7; }:global(html.dark) .brand-mark i { border-color:#fffdf7; }:global(html.dark) .settings-trigger { border-color:rgba(255,253,247,.3);background:rgba(23,42,69,.8); }:global(html.dark) .settings-panel { background:rgba(23,42,69,.99); }:global(html.dark) .settings-intro { color:#fffdf7; }:global(html.dark) .setting-row { color:#fffdf7;border-color:rgba(255,253,247,.22); }:global(html.dark) .segmented { border-color:rgba(255,253,247,.25); }:global(html.dark) .segmented button { color:rgba(255,253,247,.64); }:global(html.dark) .crossword-frame { background-color:#213a5d;border-color:#e6a527; }:global(html.dark) .crossword-cell { background:#fffdf7; }:global(html.dark) .outer-ring { stroke:#fffdf7; }:global(html.dark) .inner-ring { stroke:#fffdf7; }:global(html.dark) .core-mark { fill:#fffdf7; }:global(html.dark) .core-word,:global(html.dark) .core-caption { fill:rgba(255,253,247,.55); }:global(html.dark) .core-word.active-core { fill:#e6a527; }:global(html.dark) .letter-node>circle { fill:#172a45;stroke:#fffdf7; }:global(html.dark) .letter-node.active>circle { fill:#e6a527;stroke:#e6a527; }:global(html.dark) .letter-node.active text { fill:#172a45; }:global(html.dark) .completion-result small { color:rgba(255,253,247,.74); }
