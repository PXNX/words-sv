...
<section class="home-view" dir={interfaceDirection} lang={settings.interfaceLocale} aria-label={labels.homeTitle}>
  ...
  {:else}
    ...
    <div class="home-games">
      ...
      <article><span class="home-icon" aria-hidden="true"><IconFactCheck /></span><h2>Crossword + Definitions</h2><p>Same crossword, but tap any square for that word's definition.</p><button type="button" onclick={() => void goto('/circle/definitions')}>{labels.homePlay}</button></article>
...
<style>
  ...
  .home-games { display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:.6rem; }.home-games article { position:relative;display:grid;grid-template-rows:auto auto 1fr auto;justify-items:center;gap:.3rem;padding:1rem .6rem .8rem;border-radius:1.15rem;border:1px solid rgba(23,42,69,.14);border-top:3px solid #172a45;background:#fffdf7;box-shadow:0 8px 20px rgba(23,42,69,.08);text-align:center; }.home-games article:nth-child(2) { border-top-color:#e6a527; }.home-games article:nth-child(3) { border-top-color:#34824d; }.home-icon { display:grid;place-items:center;width:2.6rem;height:2.6rem;border:1px solid rgba(23,42,69,.24);border-radius:50%;background:rgba(230,165,39,.14);color:#a45e38; }.home-icon :global(svg) { width:1.3rem;height:1.3rem; }.home-title-row { display:contents; }.home-games h2 { margin:0;color:#172a45;font-family:'DM Serif Display',serif;font-size:.98rem;font-weight:400;line-height:1.15; }.home-games p { margin:0;color:#596477;font-family:'DM Sans',sans-serif;font-size:.6rem;font-weight:700;line-height:1.35; }.home-games button:not(.home-tutorial-link) { width:100%;min-height:2.1rem;margin-top:.2rem;border:1px solid #172a45;border-radius:999px;background:#172a45;color:#fffdf7;font-family:'DM Sans',sans-serif;font-size:.56rem;font-weight:900;letter-spacing:.08em;text-transform:uppercase; }.home-games button:active,.home-language-choice button:active { transform:scale(.97); }.home-tutorial-link { position:absolute;top:.5rem;right:.5rem;display:grid;place-items:center;width:1.35rem;height:1.35rem;flex:none;padding:0;border:1px solid rgba(23,42,69,.3);border-radius:50%;background:#fffdf7;color:#a45e38; }.home-tutorial-link :global(svg) { width:.78rem;height:.78rem; }.home-tutorial-link:active { transform:scale(.94); }
