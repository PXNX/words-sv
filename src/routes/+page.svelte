  ...
  const solvedCells = $derived.by(() => {
    ...
    return keys;
	  ...
	  $effect(() => { document.documentElement.dataset.theme = theme; document.documentElement.classList.toggle('dark', theme === 'dark'); localStorage.setItem('wordcircle-theme', theme); });
		...
		$effect(() => {
			const timeout = window.setTimeout(() => (appLoading = false), 440);
...
<svelte:window onpointerup={endSwipe} onpointercancel={endSwipe} />
	...
	<main class="game-shell">
	  <section class="game-paper" aria-label={labels.label}>
		{#if appLoading}
			<div class="startup-loader" role="status" aria-label="Loading WordCircle">
				<span class="brand-mark startup-mark" aria-hidden="true"><i></i><b></b></span>
				<span class="sr-only">Loading WordCircle</span>
			</div>
		{/if}
    {#if tutorialOpen}
...
<style>
  ...
  .startup-loader { position:absolute;z-index:120;inset:0;display:grid;place-items:center;background:rgba(255,253,247,.96); }.startup-mark { width:58px;height:58px;animation:startup-spin .82s linear infinite; }.startup-mark i,.startup-mark b { width:34px;height:34px;border-width:3px; }.startup-mark i { top:2px;left:2px; }.startup-mark b { right:2px;bottom:2px; }
  ...
  @media (prefers-reduced-motion:reduce) { .startup-mark,.letter-node:not(.active),.settings-panel,.crossword-cell.solved,.completion-inline { animation:none; } }
</style>
