...
Production commit `3d7f7fd` completed successfully on Vercel. The same direct mobile suite passed against `https://words-sv.vercel.app`: it exercised the Wordle tutorial and repeated-letter marks, the ẞ key, the persisted sound setting, the vocabulary wrong/correct feedback and next-card progression, the portrait guard, and the direct `/circle` view. The deployed `/unknown-trail` response showed the branded 404 recovery copy, while `/manifest.webmanifest` and `/sw.js` each returned HTTP 200.

The revised local mobile suite opened the root route with cleared storage and rendered the first-run learning-language card, then displayed all three selectable training games after a language choice. Wordle continued to show only its dedicated guided tutorial for a first-time route. The settings overlay contained exactly one replay control for the active Wordle tutorial. Vocabulary validation covered a correct definition answer that remained visible until Continue, a subsequent audio-only card with the large speaker and Listen control, and a wrong answer that marked the selected option red and the correct option green before Continue. The local review record verified a one-day first success interval, two days after the next success, and a reset to one day after a mistake. Landscape retained a centered portrait game composition with no orientation message. Local tests (22), Svelte diagnostics, and the static Bun PWA build passed.

The follow-up local mobile validation confirmed the Home action routes Wordle back to the root selector and that Settings has no mode picker. An invalid `ZZZZZ` Wordle entry triggered the rejection notice and cleared the active row. The light-theme vocabulary word computed to navy (`rgb(23, 42, 69)`), avoiding the previous invisible white-on-paper state; the audio-only card did not show an unavailable status solely because the initial browser voice list was empty. At 844×390, screenshots showed the app centered as an upright narrow portrait panel rather than rotated sideways. Svelte diagnostics, all 22 regression tests, and the isolated mobile interaction suite passed.

The dedicated `/settings` route rendered the former persistent preferences as a full page: game language and level, content options, interface locale, theme, vibration, sound, the active Wordle tutorial replay, share link, and repository link. The control at top left is now an icon-only accessible Home action. Direct mobile validation navigated from Wordle to `/settings`, persisted the sound setting, returned Home, and retained all earlier vocabulary, Wordle invalid-row, and upright-orientation checks. The static PWA build emitted a `/settings` entry alongside the manifest and service worker.

Production commit `f9c2fef` completed successfully on Vercel. The production mobile suite confirmed icon-only Home navigation, the direct `/settings` page, persisted sound preferences, invalid Wordle row clearing, vocabulary answer feedback, and the upright portrait fallback. Production requests for `/settings`, `/manifest.webmanifest`, and `/sw.js` each returned HTTP 200.

The refined local mobile vocabulary flow showed an inline check icon on the correct option after a correct answer. On a wrong answer, the selected option displayed a cross while the true option displayed a check. The redundant standalone speaker ornament was absent on the audio card; the Listen control retained its own speaker icon. The textual result line above Continue was removed. Svelte diagnostics, all 22 tests, the direct mobile suite, and the static PWA build passed.

Production commit `99f7885` completed successfully on Vercel. The deployed mobile validation confirmed the check icon on a correct vocabulary option, both cross and check icons after a wrong choice, no standalone speaker ornament above Listen, and no feedback sentence above Continue. The full existing mobile interaction suite remained green.
